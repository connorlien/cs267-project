...
#ifndef BATCH_BLOCK_SIZE
#define BATCH_BLOCK_SIZE 136
#endif

#ifndef CHANNEL_BLOCK_SIZE
...
static void pw_conv(double *X, double *F_1D, double *O, int B, int H_in, int W_in, int C_in, int C_out)
{
    ...
    for (int b = 0; b < B; b += 1)
    {
        ...
        for (int c = 0; c < C_in; c += 1)
        {
            ...
            {
                // Do 2D Convolution channelwise
                double *curr_channel = curr_img + C_in * c; // CHECK
                ...
                for (int w = 0; w < W_in; w += 1)
                {
                    ...
                    {
                        // PTR TO CURRENT POSITION IN FILTER
                        double *f_curr = F_1D + (c * C_out + f);

                        // PTR TO INPUT POSITION
                        double *curr_inp = curr_channel + row_major(h, w, W_in);

                        // PTR TO INPUT POSITION
                        double *curr_out_xy = curr_out + mat_size * (c * C_out + f) + row_major(h, w, W_in);
