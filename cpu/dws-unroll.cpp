...
static void dw_conv_unroll_2(float *X, float *F_DW, float *O, int B, int H_in, int W_in, int C_in, int H_f, int W_f, int N_dw, int H_out, int W_out, int stride_h, int stride_w)
{
                ...
                for (int h = 0; h < H_out; h += 1)
                {
                    for (int w = 0; w < W_out; w += 1)
                    {
                        ...
                        for (int h_f = 0; h_f < H_f; h_f += 1)
                        {
                            ...
                            float *f_curr = F_DW + f_size * (c * N_dw + f) + row_major(h_f, 0, W_f);

                            // PTR TO INPUT POSITION
                            int h_curr = h_f + stride_h * h;
                            int w_curr = stride_w * w;
                            ...
                            for (int w_f = 0; w_f < W_f / 2 * 2; w_f += 2)
                            {
                                *curr_out_xy += *f_curr * *curr_inp
                                                 + *(f_curr + 1) * *(curr_inp + 1);
                            }
                            for (int w_f = W_f / 2 * 2; w_f < W_f; w_f += 1)
                            {
                                *curr_out_xy += *(f_curr + w_f) * *(curr_inp + w_f);
                            }
                        }
                    }
                }
...
static void dw_conv_unroll_3(float *X, float *F_DW, float *O, int B, int H_in, int W_in, int C_in, int H_f, int W_f, int N_dw, int H_out, int W_out, int stride_h, int stride_w)
{
    int mat_size = W_in * H_in;
    int f_size = W_f * H_f;
    int img_size = mat_size * C_in;
                ...
                for (int h = 0; h < H_out; h += 1)
                {
                    for (int w = 0; w < W_out; w += 1)
                    {
                        ...
                        for (int h_f = 0; h_f < H_f; h_f += 1)
                        {
                            ...
                            for (int w_f = W_f / 3 * 3; w_f < W_f; w_f += 1)
                            {
                                *curr_out_xy += *(f_curr + w_f) * *(curr_inp + w_f);
                            }
                        }
...
static void dw_conv_unroll_5(float *X, float *F_DW, float *O, int B, int H_in, int W_in, int C_in, int H_f, int W_f, int N_dw, int H_out, int W_out, int stride_h, int stride_w)
{
    int mat_size = W_in * H_in;
    int f_size = W_f * H_f;
    int img_size = mat_size * C_in;
                ...
                for (int h = 0; h < H_out; h += 1)
                {
                    for (int w = 0; w < W_out; w += 1)
                    {
                        ...
                        for (int h_f = 0; h_f < H_f; h_f += 1)
                        {
                            ...
                            for (int w_f = 0; w_f < W_f / 5 * 5; w_f += 5)
                            {
                                *curr_out_xy += *f_curr * *curr_inp
                                                 + *(f_curr + 1) * *(curr_inp + 1)
                                                 + *(f_curr + 2) * *(curr_inp + 2)
                            ...
                            for (int w_f = W_f / 5 * 5; w_f < W_f; w_f += 1)
                            {
                                *curr_out_xy += *(f_curr + w_f) * *(curr_inp + w_f);
                            }
                        }
...
{
    if (W_f % 2 == 0) {
        dw_conv_unroll_2(X, F_DW, depthwise_output, B, H_in, W_in, C_in, H_f, W_f, N_dw, H_out, W_out, stride_h, stride_w);
    } else if (W_f % 5 == 0) {
        dw_conv_unroll_5(X, F_DW, depthwise_output, B, H_in, W_in, C_in, H_f, W_f, N_dw, H_out, W_out, stride_h, stride_w);
    } else {
        dw_conv_unroll_3(X, F_DW, depthwise_output, B, H_in, W_in, C_in, H_f, W_f, N_dw, H_out, W_out, stride_h, stride_w);
    }
